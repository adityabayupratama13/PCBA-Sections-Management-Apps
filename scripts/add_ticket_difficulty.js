/**
 * Migration: Add 'difficulty' column to tickets table
 * This allows storing integer ratings (1-5) for tickets.
 * Run: node scripts/add_ticket_difficulty.js
 */
...
async function run() {
  ...
  for (const [section, db] of Object.entries(SECTION_DB_MAP)) {
    try {
      await pool.query(
        `ALTER TABLE \`${db}\`.tickets ADD COLUMN difficulty INT DEFAULT 0;`
      );
      console.log(`✅ [${section}] Added difficulty column to ${db}.tickets`);
    } catch (e) {
       if (e.message.includes('Duplicate column name')) {
         console.log(`⚠️ [${section}] Column already exists in ${db}.tickets, skipped.`);
       } else {
